...
2) ~~하단에는 스크립트를 볼 수 있는 창이 있고, 이 대화창의 왼쪽 상단에는 말하고 있는 캐릭터의 이름이 뜬다. (100%)~~
...
1) 게임 화면의 왼쪽 상단에 날짜를 입력한다.
2) 오른쪽 상단에 이미지 버튼을 눌러 클릭 시 설정 화면이 뜨도록 한다.
3) 대화창과 캐릭터의 이름이 뜨는 창을 만든다.
4) 대화창에 스크립트의 글자가 차례로 나타난다.
